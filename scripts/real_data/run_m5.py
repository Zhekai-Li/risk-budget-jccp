...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the M5 real-data JCCP experiment.")
...
def _prepare_data(config_path: str, output_root: str) -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "real_data" / "prepare_m5_data.py"),
            "--config",
            config_path,
            "--output-root",
            output_root,
...
def main() -> None:
    ...
    logger = configure_case_logger("m5", paths["logs"])
    logger.info("validated M5 config and real-data output paths")
    if args.dry_run:
        return
    ...
    if not status.valid:
        if args.require_prepared_data:
            raise RuntimeError(
                "M5 processed data are not ready. Run: "
                "python scripts/real_data/prepare_m5_data.py "
                f"--config {args.config}\n{format_status(status)}"
            )
        _prepare_data(args.config, args.output_root)
        status = check_case_data("m5", cfg, raw_dir=paths["raw"], processed_dir=paths["processed"])
        if not status.valid:
            raise RuntimeError(f"M5 data preparation did not produce valid processed data:\n{format_status(status)}")
    ...
    summary = solve_m5_instance(instance, paths["results"], cfg["dca"])
    summary.to_csv(paths["tables"] / "m5_summary.csv", index=False)
    build_m5_report_assets(instance, summary, paths["results"])
    publish_split_outputs(paths["results"], "m5", args.output_root)
    logger.info("M5 real-data experiment complete: %s", paths["results"])
