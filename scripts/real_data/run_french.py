...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Kenneth French real-data JCCP experiment.")
...
def _prepare_data(config_path: str, output_root: str) -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(REPO_ROOT / "scripts" / "real_data" / "prepare_french_data.py"),
            "--config",
            config_path,
            "--output-root",
            output_root,
...
def main() -> None:
    ...
    logger = configure_case_logger("french", paths["logs"])
    logger.info("validated French config and real-data output paths")
    if args.dry_run:
        return
    ...
    if not status.valid:
        if args.require_prepared_data:
            raise RuntimeError(
                "French processed data are not ready. Run: "
                "python scripts/real_data/prepare_french_data.py "
                f"--config {args.config}\n{format_status(status)}"
            )
        _prepare_data(args.config, args.output_root)
        status = check_case_data("french", cfg, raw_dir=paths["raw"], processed_dir=paths["processed"])
        if not status.valid:
            raise RuntimeError(f"French data preparation did not produce valid processed data:\n{format_status(status)}")
    ...
    instance = build_instance(
        ...
        target_return_fraction=cfg["target_return_fraction"],
    )
    summary = solve_french_instance(instance, paths["results"], cfg["dca"])
    summary.to_csv(paths["tables"] / "french_summary.csv", index=False)
    build_french_report_assets(instance, summary, paths["results"])
    publish_split_outputs(paths["results"], "french", args.output_root)
    logger.info("French real-data experiment complete: %s", paths["results"])
